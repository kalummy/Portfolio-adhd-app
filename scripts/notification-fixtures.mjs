...
const retentionStyles = styles.match(/\.notifications-retention-note\s*\{([^}]+)\}/)?.[1];
assert.ok(retentionStyles);
assert.doesNotMatch(retentionStyles, /position:\s*(fixed|absolute|sticky)|transform:|white-space:\s*nowrap/);
assert.match(screen, /<footer className="notifications-retention-note">/);
assert.match(repository, /\.order\("fired_at", \{ ascending: false \}\)/);
...
assert.match(settingsPage, /<NotificationSettingsScreen\s+e2eAction=\{offer \?/);
assert.match(settingsPage, /const offer = await getCurrentE2EOffer\(\)/);
