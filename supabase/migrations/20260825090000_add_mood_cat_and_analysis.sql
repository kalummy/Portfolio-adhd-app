alter table public.mood_records
  ...
  add column if not exists clinic_phrase text,
...
do $$
begin
  if not exists (select 1 from pg_catalog.pg_constraint where conname = 'mood_records_cat_id_check' and conrelid = 'public.mood_records'::regclass) then
    alter table public.mood_records add constraint mood_records_cat_id_check
      check (cat_id is null or cat_id in ('white', 'calico', 'tuxedo', 'rainbow', 'sunglasses'));
  end if;
  if not exists (select 1 from pg_catalog.pg_constraint where conname = 'mood_records_analysis_status_check' and conrelid = 'public.mood_records'::regclass) then
    alter table public.mood_records add constraint mood_records_analysis_status_check
      check (analysis_status is null or analysis_status = 'completed');
  end if;
  if not exists (select 1 from pg_catalog.pg_constraint where conname = 'mood_records_analysis_consistency_check' and conrelid = 'public.mood_records'::regclass) then
    alter table public.mood_records add constraint mood_records_analysis_consistency_check
      check (
        (analysis_status is null and analysis_result is null and analysis_version is null and analysis_model is null and analysis_created_at is null)
        or
        (analysis_status = 'completed' and analysis_result is not null and analysis_version is not null and analysis_model is not null and analysis_created_at is not null)
      );
  end if;
end
$$;
  ...
  loop
    if (v_mood.details is not null and pg_catalog.jsonb_typeof(v_mood.details) <> 'object')
      ...
      or (
        ...
        and v_mood.cat_id not in ('white', 'calico', 'tuxedo', 'rainbow', 'sunglasses')
      )
      or not (
        (
        ...
        or (
          ...
          and pg_catalog.coalesce(pg_catalog.jsonb_typeof(v_mood.analysis_result) = 'object', false)
