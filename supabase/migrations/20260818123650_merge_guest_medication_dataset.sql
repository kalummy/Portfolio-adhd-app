create table public.guest_dataset_claims (
  dataset_id text primary key,
  user_id uuid not null references auth.users(id) on delete cascade,
  claimed_at timestamptz not null default now()
);

create index guest_dataset_claims_user_claimed_at_idx
on public.guest_dataset_claims (user_id, claimed_at);

alter table public.guest_dataset_claims enable row level security;

revoke all on table public.guest_dataset_claims from public, anon, authenticated;
grant select, insert on table public.guest_dataset_claims to authenticated;

create policy "Users can read their own guest dataset claims"
on public.guest_dataset_claims
for select
to authenticated
using ((select auth.uid()) = user_id);

create policy "Users can create their own guest dataset claims"
on public.guest_dataset_claims
for insert
to authenticated
